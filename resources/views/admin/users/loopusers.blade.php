  ...
  <div class="table-responsive">
      <table id="demo-foo-addrow" class="table m-t-30 no-wrap table-hover contact-list" data-page-size="10">
          ...
          <tbody>
            @foreach($ds as $row) 
              <tr>
                  <td>{{$row->id}}</td>
                  <td>
                      <a href="javascript:void(0)"><img src="{{$row->avatar}}" alt="user" width="40" class="rounded-circle" /> {{$row->name}}</a>
                  </td>
                  <td>{{$row->email}}</td>
                  <td>{{$row->phone}}</td>
                  <td>
                  ...
                  <td>
                    <form action="users/destroy/{{$row->id }}" method="get">
                      {{ csrf_field()}}
                      {!! method_field('delete')!!}
                      <button type="submit" class="btn btn-sm btn-icon btn-pure btn-outline delete-row-btn" onclick="return confirm('Bán Muốn Xóa Tài khoản Này?')"><i class="ti-close" aria-hidden="true"></i></button>
                    </form>
                  </td>
              </tr>
              @endforeach
          ...
          <tfoot>
              <tr>
                  <td colspan="2">
                      <button type="button" class="btn btn-info btn-rounded" data-toggle="modal" data-target="#add-contact">Thêm Tài Khoản</button>
                  </td>
                                  ...
                                  <form class="form-horizontal form-material" action="users/store" method="get">
                                      <div class="form-group">
                                          <div class="col-md-12 m-b-20">
                                              <input type="text" name="name" class="form-control" placeholder="Tên"> </div>
                                          <div class="col-md-12 m-b-20">
                                              <input type="text" name="email" class="form-control" placeholder="Email"> </div>
                                          <div class="col-md-12 m-b-20">
                                              <input type="text" name="phone" class="form-control" placeholder="Số điện thoại"> </div>
                                          <div class="col-md-12 m-b-20">
                                              <input type="password" name="password" class="form-control" placeholder="Password"> </div>
                                          <div class="col-md-12 m-b-20">
                                              <input type="text" name="address" class="form-control" placeholder="Địa chỉ"> </div>
                                          <div class="col-md-12 m-b-20">
                                          <label class="rdiobox">
                                        ...
                                        <label class="rdiobox">
                                          ...
                                          <div class="col-md-12 m-b-20">
                                              <label class="rdiobox">
                                                <input name="avatar" type="radio" value="upload/nam.png" checked>
                                                <span>Nam</span>
                                            </label> &nbsp; 
                                            <label class="rdiobox">
