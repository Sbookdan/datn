          ...
          <tbody>
            @foreach ($ds as $row)
              <tr>
                  ...
                  <td>{{ $row->name}}</td>
                  <td>                         
                    <form method="get" action="publisher/destroy/{{$row->id }}"> 
                    {{ csrf_field()}} {!! method_field('delete') !!}
                    <a href="publisher/edit/{{$row->id}}" class="btn btn-primary"> Chỉnh </a>
              ...
              @endforeach
              <tr>
                <a type="button"href="publisher/create" class="btn btn-info btn-rounded" >Thêm Mới</a>
              </tr>
          </tbody>
      </table>
  </div>
